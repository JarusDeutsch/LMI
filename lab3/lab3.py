    ...
    def paintEvent(self, event) -> None:
        ...
        if not self._points:
            ...
            painter.drawText(
                ...
                "Введіть параметри і натисніть «Обчислити».",
            )
            return
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Лабораторна робота 3: Графік функції")
        self.resize(900, 600)
        ...
        rb_trig = QRadioButton("y = 2·sin(x) − cos(x)")
        rb_trig.setChecked(True)
        rb_trig.toggled.connect(self._on_trig_selected)

        rb_pow = QRadioButton("y = (2^x + 10) / 4 + 9 / 2^(x − 2)")
        rb_pow.toggled.connect(self._on_pow_selected)

        func_layout.addWidget(rb_trig)
        func_layout.addWidget(rb_pow)
        ...
        self.calc_button = QPushButton("Обчислити таблицю та побудувати графік")
